...
import { ValidationError, ValidatorFunction, ValidationUtils, Validators } from './validation';

// Metadata storage for decorators
const validationMetadata = new WeakMap<any, Record<string, ValidatorFunction[]>>();
const sanitizationMetadata = new WeakMap<any, Record<string, (value: any) => any>>();
 ...
 */
export function ValidateEnum<T>(allowedValues: T[], required = false, message?: string) {
 ...
 */
export function ValidateString(minLength = 0, maxLength = Infinity, required = false, message?: string) {
  return validate(Validators.string(minLength, maxLength, { required, message }));
}
...
export function validateParams(target: any, propertyName: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = async function (...args: any[]) {
    ...
    const sanitizers = sanitizationMetadata.get(target.constructor.prototype) || {};
...
export class ValidationChain {
  ...
  string(minLength = 0, maxLength = Infinity, required = false, message?: string): this {
    this.validators.push(Validators.string(minLength, maxLength, { required, message }));
    return this;
  }
