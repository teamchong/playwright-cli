...
export class Validators {
    ...
    return (value: any, fieldName: string): ValidationResult => {
      ...
      try {
        ...
        if (!['http:', 'https:'].includes(url.protocol)) {
          errors.push(options.message || `${fieldName} must be a valid HTTP or HTTPS URL`);
        }
  ...
  static port(options: ValidatorOptions = {}): ValidatorFunction {
    return (value: any, fieldName: string): ValidationResult => {
      ...
      if (isNaN(portNum)) {
        ...
        errors.push(options.message || `${fieldName} must be between 1 and 65535`);
      }
   ...
   */
  static enum<T>(allowedValues: T[], options: ValidatorOptions = {}): ValidatorFunction {
    return (value: any, fieldName: string): ValidationResult => {
      const errors: string[] = [];
   ...
   */
  static string(minLength = 0, maxLength = Infinity, options: ValidatorOptions = {}): ValidatorFunction {
    return (value: any, fieldName: string): ValidationResult => {
      const errors: string[] = [];
      ...
      if (str.length < minLength) {
        errors.push(options.message || `${fieldName} must be at least ${minLength} characters`);
      }

      if (str.length > maxLength) {
        errors.push(options.message || `${fieldName} must be no more than ${maxLength} characters`);
      }
   ...
   */
  static custom(validatorFn: (value: any) => boolean, errorMessage: string): ValidatorFunction {
    return (value: any, fieldName: string): ValidationResult => {
...
export class ValidationUtils {
  ...
  static validateObject<T>(
    ...
    for (const [fieldName, validators] of Object.entries(schema)) {
      ...
      const result = this.validateValue(value, fieldName, validators as ValidatorFunction[]);
