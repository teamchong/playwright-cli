 ...
 */
export function createScriptContext(page: any, browserContext: any, browser: any) {
  return {
    ...
    fill: async (selector: string, value: string) => await page.fill(selector, value),
    ...
    html: async (selector?: string) => selector ? await page.innerHTML(selector) : await page.content(),
    ...
    waitFor: async (selector: string, options?: any) => await page.waitForSelector(selector, options),
    waitForText: async (text: string) => await page.waitForFunction(`document.body.innerText.includes('${text}')`),
    ...
    evalOn: async (selector: string, script: string) => await page.$eval(selector, script),
    evalAll: async (selector: string, script: string) => await page.$$eval(selector, script),
    
    // Page information helpers
    url: () => page.url(),
    title: async () => await page.title(),
    screenshot: async (path?: string) => await page.screenshot(path ? { path } : {}),
    ...
    select: async (selector: string, values: string | string[]) => await page.selectOption(selector, values),
    upload: async (selector: string, files: string | string[]) => await page.setInputFiles(selector, files),
    ...
    setCookie: async (name: string, value: string, options?: any) => 
      await browserContext.addCookies([{ name, value, url: page.url(), ...options }]),
    ...
    typeAndSubmit: async (inputSelector: string, text: string, submitSelector?: string) => {
...
export async function executeWithSimplifiedContext(
  ...
  const ctx = createScriptContext(page, browserContext, browser)
  ...
  const executeCode = new AsyncFunction(
    ...
    'text', 'html', 'value', 'isVisible', 'isEnabled', 'count',
    'waitFor', 'waitForText', 'waitForUrl', 'sleep',
    'eval', 'evalOn', 'evalAll',
    'url', 'title', 'screenshot', 'pdf',
    'check', 'uncheck', 'select', 'upload',
    'newTab', 'closeTab',
    'setStorage', 'getStorage', 'clearStorage',
    'setCookie', 'getCookies', 'clearCookies',
    'log', 'debug',
    'fillForm', 'clickAndWait', 'typeAndSubmit', 'scrollTo',
    'extractText', 'extractLinks', 'extractTable',
  ...
  return await executeCode(
    // Core objects
    ctx.page, ctx.context, ctx.browser, consoleWrapper,
    // All helpers
    ctx.goto, ctx.back, ctx.forward, ctx.reload,
    ctx.click, ctx.type, ctx.fill, ctx.press, ctx.hover,
    ctx.text, ctx.html, ctx.value, ctx.isVisible, ctx.isEnabled, ctx.count,
    ctx.waitFor, ctx.waitForText, ctx.waitForUrl, ctx.sleep,
    ctx.eval, ctx.evalOn, ctx.evalAll,
    ctx.url, ctx.title, ctx.screenshot, ctx.pdf,
    ctx.check, ctx.uncheck, ctx.select, ctx.upload,
    ctx.newTab, ctx.closeTab,
    ctx.setStorage, ctx.getStorage, ctx.clearStorage,
    ctx.setCookie, ctx.getCookies, ctx.clearCookies,
    ctx.log, ctx.debug,
    ctx.fillForm, ctx.clickAndWait, ctx.typeAndSubmit, ctx.scrollTo,
    ctx.extractText, ctx.extractLinks, ctx.extractTable
  )
}