...
describe('Validators', () => {
  ...
  describe('enum validator', () => {
    ...
    it('should reject disallowed values', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('waitUntil must be one of: load, domcontentloaded, networkidle');
    });
  ...
  describe('validateValue', () => {
    it('should run multiple validators on a single value', () => {
      const validators = [
        ...
        Validators.custom(value => value.includes('@'), '{field} must contain @')
      ];

      const result = ValidationUtils.validateValue('user@example.com', 'email', validators);
...
describe('ValidationError', () => {
  it('should create error with multiple messages', () => {
    ...
    expect(error.message).toBe('Validation failed: url is required, port must be a number');
