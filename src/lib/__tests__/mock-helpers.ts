...
export function mockBrowserHelper() {
  ...
  return {
    ...
    withBrowser: vi.fn((port: number, action: (browser: Browser) => Promise<any>) =>
      action(mockBrowser)
    ),
...
export const testFixtures = {
  ...
  consoleMessages: [
    { type: 'log', text: 'Test log message', timestamp: '2024-01-01T00:00:00.000Z' },
    { type: 'error', text: 'Test error message', timestamp: '2024-01-01T00:00:01.000Z' },
    { type: 'warn', text: 'Test warning message', timestamp: '2024-01-01T00:00:02.000Z' }
  ],
 ...
 */
export function createCommandArgs(command: string, args: string[] = [], options: Record<string, any> = {}) {
  return {
    command,
    args,
    options,
...
export function mockBrowserConnection(isConnected: boolean = true) {
  ...
  return {
    ...
    getPages: vi.fn(() => Promise.resolve(isConnected ? [
