...
describe('SessionManager', () => {
  ...
  describe('saveSession', () => {
    it('should save session data', async () => {
      ...
      vi.mocked(BrowserHelper.withBrowser).mockImplementation(async (port, action) => {
  ...
  describe('loadSession', () => {
    it('should load session data', async () => {
      ...
      expect(mockPage.setViewportSize).toHaveBeenCalledWith(sessionData.viewportSize);
    ...
    it('should throw error when session not found', async () => {
      ...
      await expect(SessionManager.loadSession('nonexistent', 9222)).rejects.toThrow(
    ...
    it('should handle error during load', async () => {
      vi.mocked(existsSync).mockReturnValue(true);
      vi.mocked(readFileSync).mockReturnValue(JSON.stringify({ invalid: 'data' }));

      vi.mocked(BrowserHelper.withBrowser).mockRejectedValue(new Error('Browser error'));
  ...
  describe('listSessions', () => {
    it('should list all sessions sorted by update time', () => {
      vi.mocked(existsSync).mockReturnValue(true);
      vi.mocked(readdirSync).mockReturnValue(['session1.json', 'session2.json', 'other.txt'] as any);
