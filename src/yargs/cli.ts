...
export function createCli(argv?: string[]) {
  const cli = yargs(argv || hideBin(process.argv))
    ...
    .demandCommand(process.env.NODE_ENV === 'test' ? 0 : 1, 'You need at least one command')
    .recommendCommands()
    .strict()
    .help()
    .alias('h', 'help')
    ...
    .middleware(async (argv) => {
      const { globalMiddlewareChain } = await import('./middleware/global-options');
 ...
 */
export function registerCommand(cli: ReturnType<typeof createCli>, command: CommandModule) {
