...
describe('Command Builder', () => {
  ...
  describe('createCommand', () => {
    ...
    it('should handle errors properly', async () => {
      const command = createCommand({
        metadata: { name: 'error', category: 'utility', description: 'Error test' },
        command: 'error',
        describe: 'Error test',
        builder: (yargs) => yargs,
        handler: async () => {
    ...
    it('should run validation if provided', async () => {
      ...
      const command = createCommand({
        metadata: { name: 'validate', category: 'utility', description: 'Validate test' },
        command: 'validate',
    ...
    it('should output JSON on error when json flag is set', async () => {
      const command = createCommand({
        metadata: { name: 'json-error', category: 'utility', description: 'JSON error test' },
        command: 'json-error',
      ...
      const argv = { port: 9222, json: true, _: ['json-error'], $0: 'test' } as any;
  ...
  describe('createLogger', () => {
    ...
    it('should create a logger that respects quiet mode', () => {
      const logger = createLogger({ port: 9222, quiet: true } as BaseCommandOptions);
    ...
    it('should create a logger that outputs JSON when json flag is set', () => {
      const logger = createLogger({ port: 9222, json: true } as BaseCommandOptions);
    ...
    it('should show debug messages only in verbose mode', () => {
      ...
      const verboseLogger = createLogger({ port: 9222, verbose: true } as BaseCommandOptions);
  ...
  describe('modifierMiddleware', () => {
    it('should parse modifier flags into an array', () => {
      ...
      expect(argv.modifiers).toEqual(['Shift', 'Control', 'Meta', 'ControlOrMeta']);
    });
