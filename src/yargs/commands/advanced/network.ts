  ...
  handler: async (cmdContext) => {
    try {
      ...
      page.on('response', response => {
        ...
        if (!argv.json) {
          const statusColor = 
            status >= 400 ? chalk.red :
            status >= 300 ? chalk.yellow :
            chalk.green;
      ...
      page.on('requestfailed', request => {
        ...
        if (!argv.json) {
          logger.info(`${chalk.red(`✗ ${method}`)} ${url} - ${failureInfo.failure}`);
        }
      ...
      if (argv.once) {
        ...
        if (!argv.json) {
          logger.info(chalk.yellow('📡 Monitoring network requests... Press Ctrl+C to stop'));
        }
