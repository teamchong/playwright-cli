...
describe('back command - REAL TESTS', () => {
  ...
  function executeWithTabId(baseCommand: string, tabId: string): { output: string; exitCode: number } {
    return runCommand(`${baseCommand} --tab-id ${tabId}`);
  }

  beforeAll(async () => {
    // Create a dedicated test tab for this test suite
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Back Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should navigate back using captured tab ID', () => {
      ...
      executeWithTabId(`${CLI} navigate "data:text/html,<h1>Page 1</h1>"`, testTabId);
      executeWithTabId(`${CLI} navigate "data:text/html,<h1>Page 2</h1>"`, testTabId);
    ...
    it('should handle no history gracefully', () => {
      ...
      const { output: newTabOutput } = runCommand(`${CLI} tabs new --url "data:text/html,<h1>Fresh Page</h1>"`);
    ...
    it('should work with multiple back navigation', () => {
      ...
      executeWithTabId(`${CLI} navigate "data:text/html,<h1>History 1</h1>"`, testTabId);
      executeWithTabId(`${CLI} navigate "data:text/html,<h1>History 2</h1>"`, testTabId);
      executeWithTabId(`${CLI} navigate "data:text/html,<h1>History 3</h1>"`, testTabId);
    ...
    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} back --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} back --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
