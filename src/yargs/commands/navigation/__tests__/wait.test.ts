...
describe('wait command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(
      `${CLI} tabs new --url "data:text/html,<div id='test-container'>Wait Test Suite Ready</div>"`
    )
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should wait with timeout using captured tab ID', () => {
      ...
      const { exitCode, output } = runCommand(
        `${CLI} wait --timeout 1000 --tab-id ${testTabId}`
      )
      expect(exitCode).toBe(0)
    ...
    it('should wait for existing element using captured tab ID', () => {
      ...
      runCommand(
        `${CLI} eval "document.body.innerHTML = '<div id=\\"waitTarget\\">Target Element</div>'" --tab-id ${testTabId}`
      )

      // Wait for the element we just added
      const { exitCode, output } = runCommand(
        `${CLI} wait "#waitTarget" --tab-id ${testTabId}`
      )
      expect(exitCode).toBe(0)
    ...
    it('should timeout waiting for non-existent element', () => {
      ...
      const { exitCode, output } = runCommand(
        `${CLI} wait "#nonExistentElement" --timeout 1000 --tab-id ${testTabId}`,
        5000
      )
    ...
    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(
        `${CLI} wait --tab-id "INVALID_ID" --timeout 1000`,
        2000
      )
      expect(exitCode).toBe(1)
    ...
    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(
        `${CLI} wait --tab-index 0 --tab-id ${testTabId} --timeout 1000`,
        2000
      )
      expect(exitCode).toBe(1)
