...
export const sessionCommand: CommandModule = {
  ...
  builder: (yargs) => {
    return yargs
      .command<SessionSaveArgs>({
        ...
        handler: async (argv) => {
          ...
          try {
            ...
            if (!argv.name || argv.name.includes('/') || argv.name.includes('\\')) {
            ...
            if (SessionManager.sessionExists(argv.name)) {
              spinner.info(chalk.yellow(`⚠️  Session '${argv.name}' already exists, updating...`));
            }

            await SessionManager.saveSession(argv.name, argv.port, argv.description);

            spinner.succeed(chalk.green(`✅ Session '${argv.name}' saved successfully`));
      ...
      .command<SessionLoadArgs>({
        ...
        handler: async (argv) => {
          ...
          try {
            ...
            spinner.succeed(chalk.green(`✅ Session '${argv.name}' loaded successfully`));
        ...
        handler: async (argv) => {
          try {
            ...
            if (!argv.force) {
              ...
              logger.warn(`⚠️  Are you sure you want to delete session '${argv.name}'?`);
      ...
      .demandCommand(1, 'Please specify a session action (save, load, list, delete)')
      .help()
      .epilogue(`
Examples:
