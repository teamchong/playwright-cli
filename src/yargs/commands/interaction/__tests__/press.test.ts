...
describe('press command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Press Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should press key using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<input id='test-input' autofocus/>" --tab-id ${testTabId}`);
      
      // Press Enter key in the specific tab
      const { exitCode } = runCommand(`${CLI} press Enter --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });
    ...
    it('should press different keys in same tab', () => {
      ...
      expect(runCommand(`${CLI} press Tab --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} press Escape --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} press Space --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should press arrow keys in specific tab', () => {
      // Navigate to test page
      runCommand(`${CLI} navigate "data:text/html,<input id='test-input' value='test text' autofocus/>" --tab-id ${testTabId}`);
      
      // Press arrow keys in the specific tab
      expect(runCommand(`${CLI} press ArrowLeft --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} press ArrowRight --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} press ArrowUp --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} press ArrowDown --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} press Enter --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} press Enter --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
