...
describe('select command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Select Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should select option using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<select id='test-select'><option value='a'>Option A</option><option value='b'>Option B</option><option value='c'>Option C</option></select>" --tab-id ${testTabId}`);
      
      // Select an option using our captured tab ID
      const { exitCode } = runCommand(`${CLI} select "#test-select" "b" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should select multiple options in multi-select', () => {
      // Navigate to page with multi-select dropdown
      runCommand(`${CLI} navigate "data:text/html,<select id='multi-select' multiple><option value='1'>One</option><option value='2'>Two</option><option value='3'>Three</option></select>" --tab-id ${testTabId}`);
      
      // Select multiple options in the same tab
      const { exitCode } = runCommand(`${CLI} select "#multi-select" "1" "3" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should work with different select elements', () => {
      // Navigate to page with multiple select elements
      runCommand(`${CLI} navigate "data:text/html,<select id='color'><option value='red'>Red</option><option value='blue'>Blue</option></select><select id='size'><option value='s'>Small</option><option value='l'>Large</option></select>" --tab-id ${testTabId}`);
      
      // Select from different dropdowns in the same tab
      expect(runCommand(`${CLI} select "#color" "blue" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} select "#size" "l" --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle non-existent element gracefully', () => {
      // Navigate to page without select element
      runCommand(`${CLI} navigate "data:text/html,<div>No select here</div>" --tab-id ${testTabId}`);
      
      // Try to select from non-existent element - command hangs on non-existent selectors
      expect(() => {
        runCommand(`${CLI} select "#nonexistent" "value" --tab-id ${testTabId}`, 2000);
      }).toThrow('Command timed out (hanging)');
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} select "#test" "value" --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} select "#test" "value" --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
