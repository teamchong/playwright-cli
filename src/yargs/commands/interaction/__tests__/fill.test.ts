...
describe('fill command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Fill Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should fill form fields using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<form><input id='name' name='name'/><input id='email' type='email'/></form>" --tab-id ${testTabId}`);
      
      // Fill multiple fields at once using our captured tab ID
      const { exitCode } = runCommand(`${CLI} fill "#name=John Doe" "#email=john@example.com" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should fill different input types in same tab', () => {
      // Navigate to page with various input types
      runCommand(`${CLI} navigate "data:text/html,<form><input id='text' type='text'/><input id='pass' type='password'/><textarea id='msg'></textarea></form>" --tab-id ${testTabId}`);
      
      // Fill different field types in the same tab
      expect(runCommand(`${CLI} fill "#text=Sample Text" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} fill "#pass=secret123" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} fill "#msg=Long message text" --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle multiple fields at once', () => {
      // Navigate to page with multiple form fields
      runCommand(`${CLI} navigate "data:text/html,<form><input id='first' name='first'/><input id='last' name='last'/><input id='company'/></form>" --tab-id ${testTabId}`);
      
      // Fill multiple fields in one command
      const { exitCode } = runCommand(`${CLI} fill "#first=John" "#last=Doe" "#company=Acme Corp" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should handle non-existent element gracefully', () => {
      // Navigate to page without form fields
      runCommand(`${CLI} navigate "data:text/html,<div>No form here</div>" --tab-id ${testTabId}`);
      
      // Try to fill non-existent element - command hangs on non-existent selectors
      expect(() => {
        runCommand(`${CLI} fill "#nonexistent=value" --tab-id ${testTabId}`, 2000);
      }).toThrow('Command timed out (hanging)');
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} fill "#test=value" --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} fill "#test=value" --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
