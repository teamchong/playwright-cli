...
describe('drag command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Drag Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should drag element using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<div id='source' draggable='true' style='width:100px;height:100px;background:blue'>Drag Me</div><div id='target' style='width:200px;height:200px;background:red;margin-top:20px'>Drop Here</div>" --tab-id ${testTabId}`);
      
      // Drag from source to target using our captured tab ID
      const { exitCode } = runCommand(`${CLI} drag "#source" "#target" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should work with different draggable elements', () => {
      // Navigate to page with multiple draggable elements
      runCommand(`${CLI} navigate "data:text/html,<div id='item1' draggable='true'>Item 1</div><div id='item2' draggable='true'>Item 2</div><div id='dropzone'>Drop Zone</div>" --tab-id ${testTabId}`);
      
      // Drag different elements in the same tab
      expect(runCommand(`${CLI} drag "#item1" "#dropzone" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} drag "#item2" "#dropzone" --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle non-existent element gracefully', () => {
      // Navigate to page without draggable elements
      runCommand(`${CLI} navigate "data:text/html,<div>No draggable elements here</div>" --tab-id ${testTabId}`);
      
      // Try to drag non-existent element - command hangs on non-existent selectors
      expect(() => {
        runCommand(`${CLI} drag "#nonexistent" "#target" --tab-id ${testTabId}`, 2000);
      }).toThrow('Command timed out (hanging)');
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} drag "#source" "#target" --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} drag "#source" "#target" --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
