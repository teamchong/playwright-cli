...
describe('click command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should click element using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<button id='test-btn'>Click Me</button>" --tab-id ${testTabId}`);
      
      // Click the button directly using our captured tab ID
      const { exitCode } = runCommand(`${CLI} click "#test-btn" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should handle non-existent element gracefully', () => {
      // Navigate to page without target element
      runCommand(`${CLI} navigate "data:text/html,<div>No button here</div>" --tab-id ${testTabId}`);
      
      // Try to click non-existent element - should fail gracefully
      const { output, exitCode } = runCommand(`${CLI} click "#nonexistent" --tab-id ${testTabId}`);
    ...
    it('should work with different input types', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<form><input id='text-input' type='text'/><div id='clickable-div'>Clickable Div</div></form>" --tab-id ${testTabId}`);
      
      // Click different elements in the same tab
      expect(runCommand(`${CLI} click "#text-input" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} click "#clickable-div" --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} click "#test" --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} click "#test" --tab-index 0 --tab-id ${testTabId}`, 2000);
