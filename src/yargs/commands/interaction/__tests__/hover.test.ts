...
describe('hover command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'>Hover Test Suite Ready</div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should hover element using captured tab ID', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<div id='test-div' style='width:100px;height:100px;background:red'>Hover Me</div>" --tab-id ${testTabId}`);
      
      // Hover the element directly using our captured tab ID
      const { exitCode } = runCommand(`${CLI} hover "#test-div" --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    });

    it('should handle non-existent element gracefully', () => {
      // Navigate to page without target element
      runCommand(`${CLI} navigate "data:text/html,<div>No hover target here</div>" --tab-id ${testTabId}`);
    ...
    it('should work with different element types', () => {
      ...
      runCommand(`${CLI} navigate "data:text/html,<button id='hover-btn'>Button</button><span id='hover-span'>Span</span>" --tab-id ${testTabId}`);
      
      // Hover different elements in the same tab
      expect(runCommand(`${CLI} hover "#hover-btn" --tab-id ${testTabId}`).exitCode).toBe(0);
      expect(runCommand(`${CLI} hover "#hover-span" --tab-id ${testTabId}`).exitCode).toBe(0);
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} hover "#test" --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} hover "#test" --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
