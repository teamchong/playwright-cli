...
describe('pdf command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'><h1>PDF Test Suite Ready</h1><p>This content will be saved as PDF</p></div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should generate PDF with default filename using captured tab ID', () => {
      const { exitCode, output } = runCommand(`${CLI} pdf --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
    ...
    it('should generate PDF with custom filename using captured tab ID', () => {
      const { exitCode, output } = runCommand(`${CLI} pdf test-page.pdf --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
      expect(output).toMatch(/PDF saved|saved PDF/i);
    ...
    it('should handle PDF format options using captured tab ID', () => {
      const { exitCode, output } = runCommand(`${CLI} pdf test-custom.pdf --format A4 --landscape --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
      expect(output).toMatch(/PDF saved|saved PDF/i);
    ...
    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} pdf --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} pdf --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
