 ...
 */
describe('snapshot command - REAL TESTS', () => {
  ...
  describe('handler execution', () => {
    it('should handle no browser session gracefully', () => {
      const { output, exitCode } = runCommand(`${CLI} snapshot`);
      expect(exitCode).toBe(1);
      expect(output).toContain('No browser');
    });

    it('should handle different port gracefully', () => {
      const { output, exitCode } = runCommand(`${CLI} snapshot --port 8080`);
      expect(exitCode).toBe(1);
      expect(output).toContain('No browser');
    });
  });
