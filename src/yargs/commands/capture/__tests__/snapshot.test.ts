...
describe('snapshot command - TAB ID FROM OUTPUT', () => {
  ...
  beforeAll(async () => {
    ...
    const { output } = runCommand(`${CLI} tabs new --url "data:text/html,<div id='test-container'><h1>Snapshot Test Suite Ready</h1><p>Accessibility tree content</p></div>"`);
  ...
  describe('direct tab targeting with captured ID', () => {
    it('should capture accessibility tree using captured tab ID', () => {
      const { exitCode, output } = runCommand(`${CLI} snapshot --tab-id ${testTabId}`);
      expect(exitCode).toBe(0);
      expect(output).toMatch(/Interactive Elements|Found \d+ interactive elements/i);
      // Snapshot should contain page structure info
      expect(output.length).toBeGreaterThan(50);
    });

    it('should handle invalid tab ID', () => {
      const { output, exitCode } = runCommand(`${CLI} snapshot --tab-id "INVALID_ID"`, 2000);
      expect(exitCode).toBe(1);
      expect(output).toMatch(/not found/i);
    });

    it('should prevent conflicting tab arguments', () => {
      const { output, exitCode } = runCommand(`${CLI} snapshot --tab-index 0 --tab-id ${testTabId}`, 2000);
      expect(exitCode).toBe(1);
