...
export class TabManager {
   ...
   */
  static runCommand(cmd: string, timeout = 5000): { output: string; exitCode: number } {
    try {
  ...
  static createTab(url?: string): string {
    ...
    const { output, exitCode } = this.runCommand(`${this.CLI} tabs new ${urlArg}`);
  ...
  static getTabs(): TabInfo[] {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('http') || trimmed.startsWith('about:') || trimmed.startsWith('chrome:')) {
      ...
      if (idMatch) {
        ...
        if (currentTab.index !== undefined && currentTab.title && currentTab.url && currentTab.id) {
          tabs.push({
  ...
  static closeTabById(tabId: string): boolean {
    ...
    const { output, exitCode } = this.runCommand(`${this.CLI} tabs close --index ${tab.index}`);
   ...
   */
  static executeWithTab(baseCommand: string, tabId: string, timeout = 5000): { output: string; exitCode: number } {
